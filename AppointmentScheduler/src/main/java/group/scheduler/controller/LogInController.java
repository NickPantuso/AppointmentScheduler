...
import group.scheduler.utilities.UserQuery;
...
public class LogInController implements Initializable {
    ...
    private boolean checkForUpcomingAppts() throws SQLException {
        ...
        for(Appointment appt : AppointmentQuery.select()) {
            ...
            if(timeDifference <= 15 && timeDifference > 0 && sZDT.getMonthValue() == ZonedDateTime.now().getMonthValue() &&
            sZDT.getDayOfMonth() == ZonedDateTime.now().getDayOfMonth() && sZDT.getYear() == ZonedDateTime.now().getYear()) {
    ...
    /**
     * Checks the default language of the user. If it's French, changes the login form to read in French.
